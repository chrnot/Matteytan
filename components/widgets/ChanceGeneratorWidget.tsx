...
export const ChanceGeneratorWidget: React.FC<ChanceGeneratorWidgetProps> = ({ isTransparent, setTransparent }) => {
    ...
    const addHistory = (type: Mode, value: string | number, detail?: string) => {
        setHistory(prev => [{ type, value, detail }, ...prev].slice(0, 5));
    };
    ...
    return (
        <div className="w-full h-full flex flex-col bg-white select-none overflow-hidden font-sans p-3">
            
            {/* 1. Header: Mode Switcher */}
            <div className="flex bg-slate-100 p-1 rounded-2xl border border-slate-200 shrink-0 mb-3 shadow-sm">
                <button 
            ...
            <div className="h-10 flex items-center justify-center shrink-0 mb-2">
                {mode === 'NUMBER' ? (
            ...
            <div 
                className="flex-1 flex flex-col items-center justify-center cursor-pointer group relative rounded-3xl border-2 border-dashed border-slate-100 hover:border-blue-100 hover:bg-slate-50/50 transition-all duration-300" 
                onClick={() => generate()}
            >
                ...
                <div className="pb-6 w-full flex justify-center">
                    <div className={`px-4 py-1.5 rounded-full bg-slate-100 border border-slate-200 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] shadow-sm transition-all ${isAnimating ? 'opacity-0 scale-95' : 'opacity-100 animate-pulse'}`}>
            ...
            <div className="mt-4 pt-4 border-t border-slate-100 shrink-0">
                ...
                <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar min-h-[50px]">
                    {history.length === 0 ? (
                        ...
                        history.map((item, i) => (
                            <div key={i} className="flex flex-col items-center shrink-0 animate-in fade-in slide-in-from-left-2 duration-300">
                                <div className="flex items-center gap-2 bg-slate-50 border border-slate-200 px-3 py-2 rounded-xl shadow-sm hover:border-blue-200 transition-colors">
                                ...
                                <span className="text-[7px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">{item.detail || item.type}</span>
                            </div>
                        ))
                    )}
                </div>
